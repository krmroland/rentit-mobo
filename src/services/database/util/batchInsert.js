...
module.exports = function batchInsert(client, tableName, batch, chunkSize = 1000) {
  let returning;
  ...
  const wrapper = assign(
    new Bluebird((resolve, reject) => {
      ...
      return Bluebird.delay(1)
        .then(getTransaction)
        .then(tr => {
          return Bluebird.mapSeries(chunks, items => tr(tableName).insert(items, returning))
            .then(result => {
              result = flatten(result || []);

              if (autoTransaction) {
                //TODO: -- Oracle tr.commit() does not return a 'thenable' !? Ugly hack for now.
                return (tr.commit(result) || Bluebird.resolve()).then(() => result);
              }
