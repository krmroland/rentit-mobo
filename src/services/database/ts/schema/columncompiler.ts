...
ColumnCompiler.prototype.compileColumn = function() {
  return (
    this.formatter.wrap(this.getColumnName()) + ' ' + this.getColumnType() + this.getModifiers()
  );
};
...
ColumnCompiler.prototype.double = ColumnCompiler.prototype.decimal = ColumnCompiler.prototype.floating =
  'float';
ColumnCompiler.prototype.timestamp = 'datetime';
ColumnCompiler.prototype.enu = function(allowed) {
  return `text check (${this.formatter.wrap(this.args[0])} in ('${allowed.join("', '")}'))`;
};

ColumnCompiler.prototype.json = 'json';
