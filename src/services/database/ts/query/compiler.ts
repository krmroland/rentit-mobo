...
class QueryCompiler {
  ...
  protected undefinedInWhereClause;
  protected undefinedBindingsInfo;
  protected emptyInsertValue = 'default values';
  protected _tableName;

  constructor(client, builder) {
    this.client = client;
    this.method = builder.method || 'select';
    this.options = builder.options;
    this.single = builder.single;
    this.grouped = groupBy(builder.statements, 'grouping');
  ...
  toSQL(method, tz) {
    this.undefinedInWhereClause = false;
    ...
    if (this.undefinedInWhereClause) {
  ...
  insert() {
    ...
    if (Array.isArray(insertValues)) {
      if (insertValues.length === 0) {
        ...
        return sql + this.emptyInsertValue;
      }
  ...
  columns() {
    ...
    return (
      ...
      sql.join(', ') +
      (this.tableName ? ` from ${this.single.only ? 'only ' : ''}${this.tableName}` : '')
    );
  }

  protected makeAggregate(stmt, { aliasSeparator = ' as ', distinctParentheses } = {}) {
  ...
  aggregate(stmt) {
    return this.makeAggregate(stmt);
  }
  ...
  where() {
    ...
    while (++i < wheres.length) {
      ...
      if (
        ...
        this.undefinedInWhereClause = true;
      }
  ...
  onWrapped(clause) {
    ...
    wrapJoin.clauses.forEach(function(wrapClause, ii) {
      ...
      const val = this[wrapClause.type](wrapClause);
      if (val) {
        sql += val;
      }
    });
  ...
  protected _formatGroupsItemValue(value) {
