...
export const makeEscape = (config = {}) => {
  ...
  const escapeFn = (val, finalEscape, ctx) => {
...
export const escapeObject = (val, finalEscape, ctx) => {
...
export const arrayToList = (array, finalEscape, ctx) => {
  let sql = '';
...
export const escapeString = (val, finalEscape?, ctx?) => {
...
export const dateToString = (date, finalEscape, ctx) => {
...
export const convertTimezone = tz => {
  ...
  if (m) {
    return (
      (m[1] == '-' ? -1 : 1) * (parseInt(m[2], 10) + (m[3] ? parseInt(m[3], 10) : 0) / 60) * 60
    );
  }
  return false;
};
