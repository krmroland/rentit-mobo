...
const { assign, uniqueId, find, identity, map, omit, invert, fromPairs } = require('lodash');
        ...
        return Bluebird.all(columns.map(column => this.getColumn(column)))
          ...
          .then(function(sql) {
            ...
            const mappedColumns = Object.keys(
              this.client.postProcessResponse(fromPairs(columns.map(column => [column, column]))),
            );
            return Bluebird.bind(this)
              .then(this.createTempTable(createTable))
              .then(this.copyData)
