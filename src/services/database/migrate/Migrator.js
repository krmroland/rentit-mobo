...
class Migrator {
  constructor(knex) {
    ...
    this.config = this.getMergedConfig(this.knex.client.config.migrations);
  ...
  latest(config) {
    ...
    this.config = this.getMergedConfig(config);
  ...
  up(config) {
    ...
    return migrationListResolver
      ...
      .then(([all, completed]) => {
        const migrationToRun = getNewMigrations(this.config.migrationSource, all, completed).slice(
          0,
          1,
        );
  ...
  _runBatch(migrations, direction, trx) {
    return (
      this._getLock(trx)
        ...
        .then(
          completed =>
            (migrations = getNewMigrations(this.config.migrationSource, migrations, completed)),
        )
          ...
          if (error instanceof LockError) {
            ...
            this.knex.client.logger.warn(
              'If you are sure migrations are not running you can release the ' +
                ...
                'table: ' +
                getLockTableNameWithSchema(this.config.tableName, this.config.schemaName),
            );
          } else {
